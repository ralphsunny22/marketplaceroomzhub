    ...
    var axilInit = {
        ...
        quantityRanger: function() {
            ...
            $('.qtybtn').on('click', function() {
                ...
                var price = $button?.closest('tr')?.find('.product-price')?.data('price'); // Assuming you add a data attribute for price
                ...
                if (price) {
                    var newSubtotal = newVal * parseFloat(price);
                    // console.log({newSubtotal});

                    // Update the closest span with the new subtotal
                    // Update the second span inside the .product-subtotal td with the new subtotal
                    $button.closest('tr').find('.product-subtotal span').eq(1).text(newSubtotal); // Target the second span (index 1)
                    // $button.closest('tr').find('.product-subtotal span').next().text(newSubtotal.toFixed(2)); // Format to 2 decimal places
                    // console.log({xyz});

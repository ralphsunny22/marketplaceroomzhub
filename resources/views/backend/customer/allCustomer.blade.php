                                    ...
                                    @if (count($customers) > 0)
                                        @foreach ($customers as $user)
                                            <tr>
                                                ...
                                                <td>{{ $user->customer->phone_number ? $user->customer->phone_number : 'N/A' }}</td>

                                                <td>{{ $user->customer->created_at->format('D, M j, Y') }}</td>

                                                <td>
                                                    <div class="btn-group">
                                                        <span class="badge bg-{{ $user->getCustomerBgColor($user->customer->status) }} pointer" data-bs-toggle="dropdown">{{ucFirst($user->customer->status)}}</span>
                                                        <div class="dropdown-menu">
                                                            @foreach ($allStatus as $status)
                                                                <a class="dropdown-item {{ $user->customer->status == $status['name'] ? 'd-none' : '' }}"

        ...
        <div class="row d-none">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">{{ucFirst($status)}} Vendors</h4>
                </div>
            </div>
        </div>
        <!-- start page title -->
                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active"><a href="javascript: void(0);">{{ucFirst($status)}} Vendors</a></li>
                        </ol>
                    </div>
                    <h4 class="page-title">{{ucFirst($status)}} Vendors</h4>
                </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <h4 class="header-title">Vendor</h4>
                        <p class="text-muted font-13 mb-4">
                            This shows the list of vendors.
                        </p>
                                            ...
                                            <tr>
                                                <td>
                                                    @if ($item->user->profile_picture)
                                                        <img src="{{ $item->user->profile_picture }}" alt="Profile Picture" style="width: 25px; height: 25px; border-radius: 5px;">
                                                    @else
                                                ...
                                                <td>{{ $item->user->phone_number ? $item->user->phone_number : 'N/A' }}</td>

                                                <td>{{ $item->business_name ? $item->business_name : 'N/A' }}</td>

                                                <td>{{ $item->user->created_at->format('D, M j, Y') }}</td>

                                                <td>
                                                    <div class="btn-group">
                                                        <span class="badge bg-{{ $item->getBgColor($item->user->status) }} pointer" data-bs-toggle="dropdown">{{ucFirst($item->user->status)}}</span>
                                                        <div class="dropdown-menu">
                                                            @foreach ($allStatus as $status)
                                                                <a class="dropdown-item {{ $item->user->status == $status['name'] ? 'd-none' : '' }}"
                                                                    {{-- href="{{ route('adminUpdateVendorstatus', ['id' => $item->id, 'status' => $status['name']]) }}" --}}
                                                                >
                                                                    {{ ucFirst($status['name']) }}
                                                                </a>
                                                            @endforeach
